...
async def check_firebase_health(settings: Optional[Settings] = None) -> FirebaseHealthStatus:
    ...
    Prüft die Erreichbarkeit der Firebase API durch einen
    Authentifizierungsversuch mit den konfigurierten Credentials.
    
    Args:
        settings: Optional Settings-Instanz. Falls None, werden die
                  aktuellen Settings verwendet.
    
    Returns:
        FirebaseHealthStatus mit dem Ergebnis der Prüfung.
    """
    if settings is None:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            # Firebase Auth REST API Endpunkt prüfen
            # Wir verwenden den identitytoolkit Endpunkt um die API-Erreichbarkeit zu prüfen
            firebase_url = (
                f"https://identitytoolkit.googleapis.com/v1/accounts:signUp"
                f"?key={settings.fb_api_key}"
            )
            
            # Sende eine Anfrage (wird fehlschlagen, aber zeigt dass die API erreichbar ist)
            ...
            # Status 400 ist OK - bedeutet die API ist erreichbar
            # (Fehler weil keine echte Registrierung)
            if response.status_code in [200, 400]:
                ...
                logger.warning(
                    f"Firebase Health Check fehlgeschlagen. "
...
async def check_and_refresh_secrets() -> tuple[FirebaseHealthStatus, bool]:
    ...
    Diese Funktion:
    1. Lädt die .env Datei neu (für geänderte Secrets)
    2. Führt einen Firebase Health-Check durch
    
    Returns:
        Tuple aus (FirebaseHealthStatus, secrets_refreshed: bool)
    """
    ...
    # Prüfen ob sich Secrets geändert haben
    secrets_changed = (
    ...
    # Health-Check mit neuen Settings durchführen
